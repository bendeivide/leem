...
variance <- function (x, rounding = 2, na.rm = FALSE, details = FALSE, grouped = TRUE) {
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      vari <- sum((x$table$PM - mean(x, na.rm = na.rm, rounding = rounding))^2 * x$table$Fi)/(x$statistics$nsample -
                                                              1)
...
sdev <- function(x, rounding = 2, na.rm = FALSE, details = FALSE,
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      desvpad <- round(sqrt((variance(x, na.rm = na.rm, rounding = rounding))), digits = rounding)
...
cv <- function(x, rounding = 2, na.rm = FALSE, details = FALSE,
  ...
  if (attr(x, "variable") == "discrete") {
    numchar <- is.numeric(x$statistics$raw_data)
    if (numchar) {
      cvariation <- round((sdev(x = x, na.rm = na.rm, rounding = rounding) / mean(x = x,
                                                                                  na.rm = na.rm, rounding = rounding)) * 100, digits = rounding)
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      ...
      cvariation <- round((sdev(x = x, na.rm = na.rm, rounding = rounding, grouped = FALSE) / mean(x = x,
                                                                                                   na.rm = na.rm, rounding = rounding, grouped = FALSE)) * 100, digits = rounding)
      resume <- list(`coeffient of variation` = cvariation, table = x$table, rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      }
...
madev <- function(x, rounding = 2, na.rm = FALSE, details = FALSE,
  ...
  if (attr(x, "variable") == "discrete") {
    numchar <- is.numeric(x$statistics$raw_data)
    if (numchar) {
      meandev <- round(sum(abs(x$statistics$raw_data - mean(x = x, na.rm = na.rm, rounding = rounding))), digits = rounding)
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      meandev <- round(sum(abs(x$table$PM - mean(x = x, na.rm = na.rm, grouped = TRUE, rounding = rounding)) * x$table$Fi), digits = rounding)
      resume <- list(`mean absolute deviation` = meandev, table = x$table, rawdata = x$statistics$raw_data)
      ...
      meandev <- round(sum(abs(x$statistics$raw_data - mean(x = x, na.rm = na.rm, grouped = FALSE, rounding = rounding))), digits = rounding)
      resume <- list(`mean absolute deviation` = meandev, table = x$table, rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      }
...
#'   mdev()
#' @export
mdev <- function(x, rounding = 2, na.rm = FALSE, details = FALSE,
                  grouped = TRUE) {
  if (!is.logical(details)) stop("The 'details' argument must be logical!",
                                 call. = FALSE, domain = "R-leem")
  if (!is.logical(grouped)) stop("The 'grouped' argument must be logical!",
  ...
  if (attr(x, "variable") == "discrete") {
    numchar <- is.numeric(x$statistics$raw_data)
    if (numchar) {
      meandev <- round(sum(x$statistics$raw_data - mean(x = x, na.rm = na.rm, rounding = rounding)), digits = rounding)
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      meandev <- round(sum((x$table$PM - mean(x = x, na.rm = na.rm, grouped = TRUE, rounding = rounding)) * x$table$Fi), digits = rounding)
      resume <- list(`mean deviation` = meandev, table = x$table, rawdata = x$statistics$raw_data)
      ...
      meandev <- round(sum(x$statistics$raw_data - mean(x = x, na.rm = na.rm, , grouped = FALSE, rounding = rounding)), digits = rounding)
      resume <- list(`mean deviation` = meandev, table = x$table, rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      }
...
#' rpois(30, 2.5) |>
#'   new_leem() |>
#'   meadev(grouped = FALSE)
#' # Example 2: Normal data
#' rnorm(50, 100, 2.5) |>
#'   new_leem(variable = 2) |>
#'   meadev()
#' @export
meadev <- function(x, rounding = 2, na.rm = FALSE, details = FALSE,
                  grouped = TRUE) {
  if (!is.logical(details)) stop("The 'details' argument must be logical!",
                                 call. = FALSE, domain = "R-leem")
  if (!is.logical(grouped)) stop("The 'grouped' argument must be logical!",
  ...
  if (attr(x, "variable") == "discrete") {
    numchar <- is.numeric(x$statistics$raw_data)
    if (numchar) {
      mediandev <- round(sum(abs(x$statistics$raw_data - median(x = x, na.rm = na.rm, rounding = rounding))), digits = rounding)
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      mediandev <- round(sum(abs(x$table$PM - median(x = x, na.rm = na.rm, grouped = TRUE, rounding = rounding)) * x$table$Fi), digits = rounding)
      resume <- list(`median absolute deviation` = mediandev, table = x$table, rawdata = x$statistics$raw_data)
      ...
      mediandev <- round(sum(abs(x$statistics$raw_data - median(x = x, na.rm = na.rm, , grouped = FALSE, rounding = rounding))), digits = rounding)
      resume <- list(`median absolute deviation` = mediandev, table = x$table, rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      }
...
medev <- function(x, rounding = 2, na.rm = FALSE, details = FALSE,
  ...
  if (attr(x, "variable") == "discrete") {
    numchar <- is.numeric(x$statistics$raw_data)
    if (numchar) {
      mediandev <- round(sum(x$statistics$raw_data - median(x = x, na.rm = na.rm, rounding = rounding)), digits = rounding)
      resume <- list(`median deviation` = mediandev, table = x$table, rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      } else {
        return(mediandev)
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      mediandev <- round(sum((x$table$PM - median(x = x, na.rm = na.rm, grouped = TRUE, rounding = rounding)) * x$table$Fi), digits = rounding)
      resume <- list(`median deviation` = mediandev, table = x$table, rawdata = x$statistics$raw_data)
...
#'
#' Compute the sample sum of squared of errors
#'
#' @param x R object (list) of class leem. Use \code{new_leem()} function.
#' @param rounding Numerical object. Rounds the values in its first argument to the specified number of decimal places (default \code{2}).
#' @param na.rm a logical value indicating whether \code{NA} values should be stripped before the computation proceeds.
...
#'
#' @export
sse <- function (x, rounding = 2, na.rm = FALSE, details = FALSE, grouped = TRUE) {
  if (!is.logical(details)) stop("The 'details' argument must be logical!",
                                 call. = FALSE, domain = "R-leem")
  if (!is.logical(grouped)) stop("The 'grouped' argument must be logical!",
                                 call. = FALSE, domain = "R-leem")
  ...
  if (!is.null(attr(x, "NA"))) return(NA)
  # Correction
  correction <- length(x$statistics$raw_data) - 1
  if (attr(x, "variable") == "discrete") {
    numchar <- is.numeric(x$statistics$raw_data)
    if (numchar) {
      vari <- round(var(x = x$statistics$raw_data, na.rm = na.rm) * correction,
                    digits = rounding)
      resume <- list(sse = vari, table = x$table,
                     rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      }
  ...
  if (attr(x, "variable") == "continuous") {
    if (grouped == TRUE) {
      sse <- sum((x$table$PM - mean(x, na.rm = na.rm, rounding = rounding))^2 * x$table$Fi)
      resume <- list(sse = sse, table = x$table, rawdata = x$statistics$raw_data)
      if (details) {
        return(resume)
      }
      else {
