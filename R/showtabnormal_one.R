.showtabnormal_one <- function(z) {
  ...
  if (!is.numeric(z) || length(z) != 1) {
    stop(gettext("Argument 'z' must be a single numeric value."))
  }

  if (z == 0) {
    stop(gettext("Choose a value greater than 0"), call. = FALSE)
  }

  z <- round(z, 2)

  zchar <- as.character(z)
  zchar1 <- substr(zchar, 1, 1)
  zchar2 <- substr(zchar, 3, 3)
  zchar3 <- substr(zchar, 4, 4)
  ...
  text(0, 0.5,
       labels = bquote(bold(.(gettext("Standard normal distribution")))),
       cex = 1.5)
  # Background of table
  ...
  if (z1  == 0) {
    ...
    if (z2 == 0.01) {
      ...
      # Table title
      text(6.1, 0.25,
           labels = bquote(bold(.(gettext("Z Table")))),
           cex = 1.5)
      ...
      # Table title
      text(6.1, 0.25,
           labels = bquote(bold(.(gettext("Z Table", domain = "R-leem")))),
           cex = 1.5)
      text(4.75,0.15, labels = bquote(bold("Z")), cex = 1.5)
    ...
    if (z2 == 0) {
      ...
      segments(4.4, 0.1, 7.9, 0.1, lwd = 2)
      segments(5.1, 0.2, 5.1, -0.3, lwd = 2)
      text(6.1,0.25, labels = bquote(bold(.(gettext("Z Table", domain = "R-leem")))),, cex = 1.5)
      text(4.75,0.15, labels = bquote(bold("Z")), cex = 1.5)
      text(5.48,0.15, labels = bquote("0.00"), cex = 1.2)
  ...
  # Title
  title(
    main = gettext("Understanding the standard normal distribution table", domain = "R-leem"),
    col.main = "blue",
    cex.main = 1.5
  )
}
