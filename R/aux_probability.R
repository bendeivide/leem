...
# RSTUDIO: Low-level function to plot the Normal distribution highlighting P(a > X > b)
...
# SHINY: Low-level function to plot the Normal distribution highlighting P(a > X > b)
plotpnormalrashiny_aux <- function(q1, q2, q, mu, sigma, rounding,
                                   dec, long.segment, col,
                                   col2, lty, main,
                                   text.size, cex.main,
                                   cex.axis, cex.lab,
...
plotpnormalrashiny <- function(q1, q2, q, mu, sigma, rounding, porcentage,
                                minimo, maximo, dec,
                                long.segment, col,
                                col2, lty, main,
...
plotpbinomialarplot <- function(q, size, prob, rounding, main = NULL){
  ...
  if (qqmin < 0) {
    ...
    title(ylab = expression(p[X](x)), xlab = "X",
          main = substitute(atop(bold(titulo),
                                 p[X](x) == frac(n*"!", x*"!"*(n-x)*"!")*p^x*(1-p)^{n-x}*","~~P(X <= t1)== sum(p[X](x), x <= t1, "")*","~~P(X >= t2)== sum(p[X](x), x >= t2, infinity)),
                            list(t1 = qqmin, t2 = qqmax, x = "x", titulo = titulo)), cex = 1)
    # legends
    legaux <- legend("topleft", bty="n", fill="red", cex = 0.8,
                     legend = substitute(P(X<=t1)+P(X>=t2)==Pr,
                                         list(t1=qqmin,t2=qqmax, Pr = Pr)))
...
# RSTUDIO: Low-level function to plot the Normal distribution highlighting P(a > X > b)
plotpnormalrbrstudio_aux <- function(q1, q2, q, mu, sigma, rounding,
                                     dec, long.segment, col,
                                     col2, lty, main,
                                     text.size, cex.main,
                                     cex.axis, cex.lab,
  ...
  q[2] <- q2
  # Plot
  plotpnormalrbplot(q, mu, sigma, rounding,
                    dec, long.segment, col,
                    col2, lty, main,
                    text.size, cex.main,
                    cex.axis, cex.lab,
...
plotpnormalrbrstudio <- function(q1, q2, q, mu, sigma, rounding,
                                 minimo, maximo, dec,
                                 long.segment, col,
                                 col2, lty, main,
                                 text.size, cex.main,
  ...
  manipulate::manipulate(
    ...
    plotpnormalrbrstudio_aux(q1, q2, q, mu, sigma, rounding,
...
# TCLTK: Low-level function to plot the Normal distribution highlighting P(a > X > b)
plotpnormalrbtcltk_aux <- function(q1, q2, q, mu, sigma, rounding,
                                   dec, long.segment, col,
                                   col2, lty, main,
                                   text.size, cex.main,
                                   cex.axis, cex.lab,
...
plotpnormalrbtcltk <- function(q1, q2, q, mu, sigma, rounding,
                               minimo, maximo, dec,
                               long.segment, col,
                               col2, lty, main,
                               text.size, cex.main,
  ...
  #./tkplotleem.R
  .tkplotleemrbnormal(q1, q2, q, mu, sigma, rounding,
                      minimo, maximo, dec,
                      long.segment, col,
                      col2, lty, main,
                      text.size, cex.main,
...
# SHINY: Low-level function to plot the Normal distribution highlighting P(a > X > b)
plotpnormalrbshiny_aux <- function(q1, q2, q, mu, sigma, rounding,
                                   dec, long.segment, col,
                                   col2, lty, main,
                                   text.size, cex.main,
                                   cex.axis, cex.lab,
...
plotpnormalrbshiny <- function(q1, q2, q, mu, sigma, rounding, porcentage,
                               minimo, maximo, dec,
                               long.segment, col,
                               col2, lty, main,
                               browser.shiny = getOption("shiny.launch.browser", interactive()),
  ...
  listres <- list(
    probability = prob,
    #./shinyplotleem.R
    process_shiny = .shinyplotleemrbnormal(
      q1, q2, q, mu, sigma, rounding,
      minimo, maximo, dec,
      long.segment, col,
      col2, lty, main,
...
plotpbinomialbrplot <- function(q, size, prob, rounding, main = NULL){
  ...
  qq <- round(q, digits = rounding)
  aux2 <- par("usr")[3]-(par("usr")[4] - par("usr")[3])/20
  Pr <- round(sum(dbinom(q[1]:q[2], size = size, prob = prob)),
              digits = rounding)
  qqmin <- qq[1]
  qqmax <- qq[2]
  # red vertical lines and points
  ...
  titulo <- gettext("Probability function plot: Binomial", domain = "R-leem")
  title(ylab = expression(p[X](x)), xlab = "X",
        main = substitute(atop(bold(titulo), p[X](x) == frac(n*"!", x*"!"*(n-x)*"!")*p^x*(1-p)^{n-x}*","~~P(t1<=~X<=~t2)== sum(p[X](x), x == t1, t2)),
                          list(t1 = qqmin, t2 = qqmax, x = "x", titulo = titulo)))
  legaux <- legend("topleft", bty="n", fill="red", cex=0.8,
                   legend = substitute(P(t1<=~X<=~t2)==Pr,
                                       list(t1=qqmin,t2=qqmax, Pr = Pr)))
  parametro <- gettext("Parameters:", domain = "R-leem")
...
plotpnbinombrplot <- function(q, size, prob, rounding, main = NULL){
  ...
  # title
  if (is.null(main)) {
    main <- substitute(atop(bold("Probability function plot: Negative Binomial"),
                            p[X](x) == (atop(k+r-1,k))*(1-p)^n*p^k*","~~P(t1<=~X<=~t2)== sum(p[X](x), x == t1, t2)),
...
# TCLTK: Low-level function to plot the Normal distribution highlighting P(X <= q)
...
# SHINY: Low-level function to plot the Normal distribution highlighting P(X <= q)
...
plotppoissonlttplot <- function(q, lambda, rounding, main = NULL){
  ...
  title(ylab = expression(p[X](x)), xlab = "X",
        main = substitute(atop(bold(titulo), p[X](x) == frac(symbol(lambda)^x*e^-symbol(lambda), x*"!")*","~~F[X](t) == sum(p[X](x), x<=t, "")),
...
# RSTUDIO: Low-level function to plot the Normal distribution highlighting P(X > q)
plotpnormalltfrstudio <- function(q, mu, sigma, rounding,
                                  minimo, maximo, dec,
                                  long.segment, col,
                                  col2, lty, main,
                                  text.size, cex.main,
...
# TCLTK: Low-level function to plot the Normal distribution highlighting P(X > q)
plotpnormalltftcltk <- function(q, mu, sigma, rounding,
                                minimo, maximo, dec,
                                long.segment, col,
                                col2, lty, main,
                                text.size, cex.main,
  ...
  #./tkplotleem.R
  .tkplotleemltfnormal(q, mu, sigma, rounding,
                       minimo, maximo, dec,
                       long.segment, col,
                       col2, lty, main,
                       text.size, cex.main,
...
# SHINY: Low-level function to plot the Normal distribution highlighting P(X > q)
plotpnormalltfshiny <- function(q, mu, sigma, rounding, porcentage,
                                minimo, maximo, dec,
                                long.segment, col,
                                col2, lty, main,
                                browser.shiny = getOption("shiny.launch.browser", interactive()),
  ...
  listres <- list(
    probability = prob,
    #./shinyplotleem.R
    process_shiny = .shinyplotleemltfnormal(
      q, mu, sigma, rounding,
      minimo, maximo, dec,
      long.segment, col,
      col2, lty, main,
...
plotppoissonltfplot <- function(q, lambda, rounding, main = NULL){
  ...
  titulo <- gettext("Probability function plot: Poisson", domain = "R-leem")
  title(ylab = expression(p[X](x)), xlab = "X",
        main = substitute(atop(bold(titulo), p[X](x) == frac(symbol(lambda)^x*e^-symbol(lambda), x*"!")*","~~S[X](t)~"="~1 - F[X](t)~"="*1 - sum(p[X](x), x<=t, "")~"="*P(X >= t2) == sum(p[X](x), x >= t2, infinity)),
                          list(t = q, t2 = q + 1, titulo = titulo)))
  grid(col = "gray90")
...
plotpbinomialltfplot <- function(q, size, prob, rounding, main = NULL){
  ...
  axis(2)
  titulo <- gettext("Probability function plot: Binomial", domain = "R-leem")
  title(ylab = expression(p[X](x)), xlab = "X", main = substitute(atop(bold(titulo), p[X](x) == frac(n*"!", x*"!"*(n-x)*"!")*p^x*(1-p)^{n-x}*","~~S[X](t)~"="~1 - F[X](t)~"="*1 - sum(p[X](x), x<=t, "")~"="*P(X >= t2) == sum(p[X](x), x >= t2, infinity)),
                                                                  list(t = q, t2 = q + 1, titulo = titulo)))
  grid(col = "gray90")
  lines(x1, probx1, type = "h", lwd = 2)
  points(x1, probx1, lwd = 2, pch = 19)
...
plotpbinomialltnplot <- function(q, size, prob, rounding, main = NULL){
  ...
  legend(rmin, legaux$text$y, bty="n", bg = "white",
         legend = substitute(parametro~ n == N ~ "," ~ p == P,
                             list(N = size, P = prob, parametro = parametro)),cex = 0.8)
}
