...
#' showsf()
#' @export
showsqf <- function(variable = "discrete", prop = NULL) {
  ...
  if (variable == "discrete") {
    ...
    x <- rmin:rmax
    lambda <- 2
    qpointx <- qpois(x, lambda = lambda, lower.tail = TRUE)
    xlim <- c(rmin, rmax)
    ylim <- c(0, 1.2)
    plot.new()
    plot.window(xlim, ylim)
    titdist <- gettext("s-Quantile Function", domain = "R-leem")
    titvar <- gettext("Discrete variable", domain = "R-leem")
    titprp <- gettext("s-Quantile Function Properties", domain = "R-leem")
